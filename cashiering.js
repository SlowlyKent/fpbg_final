    ...
    try {
        ...
        if (product.unit_of_measure === 'kg' || product.unit_of_measure === 'g') {
            if (kilogramInput) {
                ...
                if (floatValue < 1) {
                    // User entered grams as a decimal (e.g., 0.90 for 90g)
                    kilogram = floatValue * 1000; // 0.90 * 1000 = 900g
                } else {
        ...
        const effectiveQuantity = product.unit_of_measure === 'kg' || product.unit_of_measure === 'g'
            ? quantity * (kilogram / 1000)
            : quantity;

        if (effectiveQuantity > product.available_quantity) {
            alert(`Only ${product.available_quantity} ${product.unit_of_measure} available for this product!`);
            return;
        }
        ...
        if (productIndex !== -1) {
            const currentEffectiveQuantity = product.unit_of_measure === 'kg' || product.unit_of_measure === 'g'
                ? cart[productIndex].quantity * (cart[productIndex].kilogram / 1000)
...
function updateTable() {
    ...
    cart.forEach((item, index) => {
        ...
        if (item.unit_of_measure === 'kg') {
            const kgValue = item.kilogram / 1000;
            displayQuantity = `${item.quantity} x ${kgValue.toFixed(3)}kg`;
        } else if (item.unit_of_measure === 'g') {
            const gValue = item.kilogram; // already in grams
            displayQuantity = `${item.quantity} x ${parseInt(gValue)}g`;
        } else {
            displayQuantity = `${item.quantity} ${item.unit_of_measure || ''}`;
        }
        let itemTotal;
        if (item.unit_of_measure === 'kg' || item.unit_of_measure === 'g') {
...
function getNotificationTypeClass(message) {
    ...
    function selectProduct(id, name, price, unit) {
        console.log('Selected unit:', unit); // Debug: show selected unit
        const productList = document.getElementById('productList');
        const quantityInput = document.getElementById('quantity');
        const kilogramInput = document.getElementById('kilogram');
        
        // Show/hide kilogram input based on unit
        if (unit === 'kg' || unit === 'g') {
        if (kilogramInput) kilogramInput.style.display = 'block';
    } else {
        if (kilogramInput) kilogramInput.style.display = 'none';
    }
}
